...
export class ApiAuth extends QueryAuth {
  postUser = async (p: PingParams & BodyCreateUser): Promise<PingResponse> => {
    ...
    const inserUser = this.insertUser({
      name: p.name,
      email: p.email,
    ...
    const insertAddress = this.insertAddress({
      cep: p.cep,
      cidade: p.cidade,
      estado: p.estado,
      numero: p.numero,
  ...
  postLogin = async (
    ...
    const insertSession = this.insertSession({
      session_hash: stringSession,
      user_id: selectUserEmail.id,
    });
    ...
    res.setHeader(
      "Set-Cookie",
      `__Secure-sid=${stringSession}; Path=/; Secure; HttpOnly; SameSite=Lax`,
    );
  ...
  updateUser = async (body: BodyUpdateUser): Promise<PingResponse> => {
    ...
    if (!email) {
      throw new APIError(ErrCode.InvalidArgument, "Email está vazil");
    }
    let hashPassword;
    if (password) {
    ...
    const updateUser = this.updateUserData({
      name,
      email,
    });
    ...
    const updateAddress = this.updateAddress({
      ...
      numero,
      rua,
      user_id: authData.userID.id,
    });

    if (!updateAddress.changes) {
      throw new APIError(ErrCode.Internal, "Erro ao atualizar endereço");
    }
  ...
  deleteLogout = async (
    p: IncomingMessage,
    res: ServerResponse,
  ): Promise<ServerResponse | APIError> => {
    const { userID }: any = getAuthData();
    ...
    const selectSession = this.selectSessionEmail({
      email: userID,
      hash: cookie,
    });
